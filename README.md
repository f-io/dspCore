...
First prototyp build in 2014, redesigned in 2016 and the lastest version in 2021, mostly to substetude conflict materials like tantalum. The I2S input was tested with RaspberryPi 3/4/5 and ESP32S3.
...
This is a loop back measurment (DSP->D/A->A/D->DSP), the result is therefore the performance of the full chain. The readings are close to the datasheet specifications of the CS4398, which makes us confident that we are measuring the right thing, but of course there is an uncertainty without external verification.<br>
